...
1. Перед началом изменения проверить `git log` и `git status`, убедиться, какая версия CHANGELOG уже вошла в `HEAD`, и определить следующую версию.
2. Сразу создать верхний раздел следующей версии в `CHANGELOG.md`.
3. После каждой итерации правки кода дополнять тот же раздел описанием изменений.
4. Не изменять раздел версии, который уже попал в созданный коммит: он считается закрытым.
5. После создания коммита следующий набор изменений получает новый номер и новый верхний раздел.

Любое изменение кода обязательно отражать в `CHANGELOG.md` в рамках текущей незакоммиченной версии до создания коммита. Например, если в `HEAD` последняя версия `2.9.0`, новая фича оформляется как `2.10.0`, а исправление — как `2.9.1`; после коммита выбранная версия становится закрытой.

Версия имеет формат `MAJOR.MINOR.PATCH`:

- `MAJOR` — большой эпик; например `0.5.3 → 1.0.0`;
- `MINOR` — новая фича или новое пользовательское действие; например `2.9.0 → 2.10.0` или `0.1.22 → 0.2.0`;
- `PATCH` — баг, небольшая правка или документация; например `0.2.0 → 0.2.1`.

## Рабочий процесс

1. Определить слой изменения и версию.
...
7. Обновить README при необходимости.
8. Проверить обе темы и i18n для UI.
9. Запустить Gradle-проверки и `git diff --check`.
10. В результате указать изменённые файлы, проверки и ограничения.
